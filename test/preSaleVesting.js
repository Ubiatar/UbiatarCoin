...
describe("PreSaleVesting withdraw function test", () => {
    var accounts, networkId, safeMath, preSaleVestingTest, uac, stdToken, owned, uacUnsold, foundersVesting, ico
    ...
    const ICODeploy = (uacAddress, uacUnsoldAddress, founderVestingAddress, preSaleVestingTestAddress) => {
        return ICO.new(uacAddress, uacUnsoldAddress, founderVestingAddress, preSaleVestingTestAddress, {from: owner})
            .then(_ico => ico = _ico)
            .then(() => uac.setIcoContractAddress(ico.address, {from: owner}))
    ...
    it("should add a new investor", () => {
        return ICODeploy(uac.address, uacUnsold.address, foundersVesting.address, preSaleVestingTest.address)
