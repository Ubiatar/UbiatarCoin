...
contract PreSaleVesting is Owned
{
    ...
    returns (uint reclaimableTokens)
    {
        ...
        if ((uint(now) >= firstThreshold) && (investors[user].firstWithdraw == 0)) {
            investors[user].balance = investors[msg.sender].initialBalance;
    ...
    {
        return investors[user].initialBalance.sub(getReclaimableTokens(msg.sender).add(getLockedTokens(msg.sender)));
    }
